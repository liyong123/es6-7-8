...
{
    /* map数组的映射 */
    var resultJson = [{ title: 'react', star: 5 }, { title: 'vue', star: 4 }, { title: 'webpack', star: 3 }];
    var resultSelf = resultJson.map(function (item) {
        //返回新对象，指向另一块堆内存
        return {
            project: item.title,
            ability: item.star
...
{
    // 以上方式的传统做法
    var resultJsonOld = [{ title: 'react', star: 5 }, { title: 'vue', star: 4 }, { title: 'webpack', star: 3 }];
    var resultSelfOld = resultJsonOld.map(function (item) {
        var obj = {};
...

{
    ...
    */
    //1、统计字符串中每个字母出现的次数
    var str = 'abachhdbc';
    var result = str.split('').reduce(function (acc, currentV) {
    ...
    var arr = [1, 2, 3, ['abc', 4, 5, 6, ['def', 7, 8, 9]]];
    var deepFlat = function deepFlat(list) {
        return list.reduce(function (acc, curV) {
